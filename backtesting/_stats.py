...
def compute_stats(
    ...
    s['# Long Trades'] = n_long_trades = int((trades_df['Size'] > 0).sum())
    s['# Short Trades'] = n_short_trades = int((trades_df['Size'] < 0).sum())
    s['Short/Long Ratio'] = n_short_trades / (n_long_trades or np.nan)
